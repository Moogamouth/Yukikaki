class Permaloom {
	...
	async archive(options) {
		...
		if (options.i > 0) {
			...
			for (j of (await this.page.evaluate("performance.getEntriesByType(\"resource\");")).map(({name}) => name)) {await archive({"url": j, "key": options.key, "hrefs": true, "i": options.i - 1, "after": options.after});}
			if (hrefs && options.html) {
				for (j of await this.page.$$eval("a", as => as.map(a => a.href))) {await archive({"url": j, "key": options.key, "i": options.i - 1, "after": options.after, "html": true});}
			}
		}
	}
};
